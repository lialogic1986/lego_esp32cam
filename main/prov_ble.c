...
static bool prov_is_complete(void)
{
    ...
    /* total blocks = ceil(total_len/16) */
...
{
    if (g_conn_handle == BLE_HS_CONN_HANDLE_NONE || g_status_handle == 0)
        return;
    ...
    int rc = ble_gattc_notify_custom(g_conn_handle, g_status_handle, om);
    if (rc != 0)
    {
...
static esp_err_t parse_json_cfg(const uint8_t *buf, uint16_t len, prov_session_cfg_t *out)
{
    ...
    if (cJSON_IsNumber(ums))
    {
        out->unix_ms_utc = (int64_t)ums->valuedouble;
    }
    else
    {
        out->unix_ms_utc = (int64_t)strtoll(ums->valuestring, NULL, 10);
    }
                            ...
                            struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    (void)attr_handle;
    (void)arg;
    /* Мини-инфо для хоста (можно расширить) */
                              ...
                              struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    uint32_t sid = (uint32_t)tmp[0] | ((uint32_t)tmp[1] << 8) | ((uint32_t)tmp[2] << 16) | ((uint32_t)tmp[3] << 24);
    uint16_t off = (uint16_t)tmp[4] | ((uint16_t)tmp[5] << 8);
    uint16_t total = (uint16_t)tmp[6] | ((uint16_t)tmp[7] << 8);
    ...
    /* copy payload: need full flat read for payload */
    ...
    /* optional progress notify (не спамить слишком часто) */
    if (prov_is_complete())
    {
                           ...
                           struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    if (!g_cfg.apply_cb)
    {
        status_notify_str("ERROR:NO_APPLY_CB");
        return 0;
    }
...
static void advertise_start(void)
{
    ...
    /* service UUID */
    fields.uuids128 = (ble_uuid128_t *)&UUID_PROV_SVC;
